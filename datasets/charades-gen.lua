...
--
--  This generates a file gen/charades.t7 which contains the list of all
...
local function prepare(opt,labels,split)
    ...
    if split=='val_video' then
        imageClasses2 = torch.IntTensor(4000000, opt.nClasses):zero() --allocating memory
    end
    for id,label in pairs(labels) do
        ...
        if not f then 
            ...
            if split=='val_video' then
                ...
                local tmp = torch.linspace(1,N,testGAP)
                for ii = 1,testGAP do
                    local i = torch.floor(tmp[ii])
                    local impath = iddir .. '/' .. id .. '-' .. string.format('%06d',i) .. '.jpg'
                    count = count + 1
                    ...
                    for _,anno in pairs(label) do
                        for i = 1,N,GAP do
                            if (anno.s<(i-1)/FPS) and ((i-1)/FPS<anno.e) then
                                local impath = iddir .. '/' .. id .. '-' .. string.format('%06d',i) .. '.jpg'
                                table.insert(imagePaths,impath)
                                table.insert(imageClasses, 1+tonumber(string.sub(anno.c,2,-1))) -- 1-index
                                table.insert(ids,id)
                            end
                        end
                    end
